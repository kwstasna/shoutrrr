...
2026-07-10 · golden pair via CLI (`shadcn add collapsible --overwrite` against style `base-rhea`) · clean.

## Changed

- `resources/js/components/ui/collapsible.tsx` — replaced the Radix wrapper with the base-rhea variant. Primitive import now `@base-ui/react/…`; `Content`→`Panel`. The CLI reproduced this project's exact resolution (lucide icons from `IconPlaceholder`, `menuColor`/`menuAccent` translucent expansion, `cn-font-heading`→`font-heading`, `@/lib/utils` alias, `"use client"` stripped). Leftover scan `grep -n "radix-ui\|@radix-ui\|IconPlaceholder"` on this file: clean.
- Consumers (`connect-buttons.tsx`) had `CollapsibleTrigger asChild`→`render` — see project.md sweep.
