...
2026-07-10 · golden pair via CLI (`shadcn add button --overwrite` against style `base-rhea`) · clean.

## Changed

- `resources/js/components/ui/button.tsx` — replaced the Radix wrapper with the base-rhea variant. Primitive import now `@base-ui/react/…`; migrated to the real `@base-ui/react/button` primitive (supports `render` natively). The CLI reproduced this project's exact resolution (lucide icons from `IconPlaceholder`, `menuColor`/`menuAccent` translucent expansion, `cn-font-heading`→`font-heading`, `@/lib/utils` alias, `"use client"` stripped). Leftover scan `grep -n "radix-ui\|@radix-ui\|IconPlaceholder"` on this file: clean.
- Consumers using `<Button asChild>` now use `render={<…/>}`; `nativeButton={false}` added where a Button renders an `<a>`/`<Link>`. See project.md.
