...
2026-07-10 · golden pair via CLI (`shadcn add breadcrumb --overwrite` against style `base-rhea`) · clean.

## Changed

- `resources/js/components/ui/breadcrumb.tsx` — replaced the Radix wrapper with the base-rhea variant. Primitive import now `@base-ui/react/…`; `Slot`/`asChild`→`useRender`+`mergeProps` for BreadcrumbLink. The CLI reproduced this project's exact resolution (lucide icons from `IconPlaceholder`, `menuColor`/`menuAccent` translucent expansion, `cn-font-heading`→`font-heading`, `@/lib/utils` alias, `"use client"` stripped). Leftover scan `grep -n "radix-ui\|@radix-ui\|IconPlaceholder"` on this file: clean.
- `breadcrumbs.tsx` `BreadcrumbLink asChild`→`render` (no `nativeButton` — Breadcrumb parts use `useRender`, not the Button primitive).
