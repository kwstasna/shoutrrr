...
class ReplyVideoUploadController extends Controller
{
    ...
    public function store(StoreReplyVideoRequest $request, PostTargetReply $reply): JsonResponse
    {
        ...
        // tmp prefix entirely.
        $this->authorizeKey($key, (string) $reply->workspace_id);
        ...
        if ($size > Platform::maxVideoBytesCeiling()) {
            ...
            Log::warning('Rejected oversize video upload', ['reply_id' => $reply->id, 'size' => $size]);
        ...
        if (! $this->looksLikeMp4($disk, $key)) {
            Storage::disk($disk)->delete($key);
            Log::warning('Rejected non-MP4 video upload', ['reply_id' => $reply->id]);
            abort(422, 'Uploaded file is not a valid MP4 video.');
        }

        $final = 'media/'.$reply->workspace_id.'/'.Str::uuid().'.mp4';
