...
class TikTokConnector implements PublishConnector
{
    ...
    private function initPhoto(PublishContext $context, MediaUploadState $state, string $token): string
    {
        ...
        // TikTok fetches each photo from a public URL on our domain (PULL_FROM_URL).
        // PublicMediaUrl::for transcodes anything outside TikTok's allowedMime
        // (PNG/GIF) to JPEG and passes JPEG/WebP through untouched — the same
        // URL-fetch rendition path Instagram photos use.
        $urls = array_map(
            fn (PostMedia $media): string => $this->publicMediaUrl->for($media, Platform::TikTok),
            $photos,
        );
