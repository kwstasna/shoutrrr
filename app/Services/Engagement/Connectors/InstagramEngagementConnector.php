...
/**
 * Reads and writes comments on an Instagram media object via the Graph API,
 * using the linked Page access token stored on the connected account.
 * Requires `instagram_manage_comments`; without it Instagram returns 403,
...
class InstagramEngagementConnector implements EngagementConnector
{
    ...
    public function fetchReplies(ConnectedAccount $account, PostTarget $target, array $credentials, ?CarbonImmutable $since): ReplyFetchResult
    {
        ...
        foreach ((array) $response->json('data', []) as $comment) {
            ...
            $createdAtRaw = (string) ($comment['timestamp'] ?? '');
    ...
    {
        if ($media !== []) {
            return ReplyPostResult::unsupported('Instagram does not support media on comments');
        }
